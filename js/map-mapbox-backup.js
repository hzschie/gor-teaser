...
class TirolCampsMap {
    ...
    setupMapInteractions() {
        ...
        this.map.on('click', 'camps-circle', (e) => {
            ...
            this.popup
                .setLngLat(e.lngLat)
                .setHTML(feature.properties['Bezeichnung'])
                .addTo(this.map);
        });
    ...
    setMapLanguageToGerman() {
        // Set all label layers to display German names
...
class BirthplacesMap {
    ...
    initMap() {
        this.map = new mapboxgl.Map({
            ...
            center: [15, 48], // Center on Central Europe
            zoom: 3,
            scrollZoom: false, // Disabled by default, enabled with Cmd/Ctrl
            attributionControl: false
        });
    ...
    fitMapToBounds() {
        // Create bounds that include all birthplaces and Innsbruck
        const bounds = new mapboxgl.LngLatBounds();
        
        // Add Innsbruck to bounds
        bounds.extend(this.innsbruckCoords);
        
        // Add all birthplaces to bounds
        this.birthplacesData.forEach(place => {
            bounds.extend([place.lat, place.long]); // JSON has lat/long swapped, so we swap them back
        });
